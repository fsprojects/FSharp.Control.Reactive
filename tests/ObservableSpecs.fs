...
[<Test>]
let ``Xunit: When subscribing to a single value observable, OnNext and OnCompleted should be fired``() =
    Observable.Return(1) |> ``should be`` 1 false true

[<Test>]
let ``Xunit: When subscribing to an empty observable, only OnCompleted should be fired``() =
    Observable.Empty() |> ``should be`` 0 false true

[<Test>]
let ``Xunit: When subscribing to an observable that fires an exception, only OnError should be fired``() =
    Observable.Throw(Exception()) |> ``should be`` 0 true false

[<Test>]
let ``Xunit: When subscribing to an F# event, only OnNext should be called``() =
    let next = ref 0
    let error = ref false
    let completed = ref false
    let testEvent = Event<EventHandler, EventArgs>()
...
[<Test>]
let ``Xunit: When subscribing to an event, only OnNext should be fired once.``() =
    let next = ref 0
    let error = ref false
    let completed = ref false
...
[<Test>]
let ``Xunit: When subscribing to an observable that fires an exception using the ObservableBuilder, only OnError should be fired``() =
    let builder = observe {
        failwith "Test"
        yield 1 }
    builder |> ``should be`` 0 true false
...
[<Test>]
let ``Xunit: When zip is defined with the applicative, it should match the result of Observable.zip``() =
    let inline (<*>) f m = Observable.apply f m
    let inline (<!>) f m = Observable.map f m
    let a = Observable.Return 1
    let b = Observable.Return 2
...
[<Test>]
let ``Xunit: zip used in rxquery matches output from Observable.zip`` () =
    let a = Observable.ofSeq [1;2;3]
    let b = Observable.ofSeq [1;2;3]
...
[<Test>]
let ``Xunit: Test should show the stack overflow is fixed with Rx 2 beta``() =
    let test() =
        let rec g x = observe {
            yield x
            if x < 100000 then
...
[<Test>]
let ``Xunit: RxQueryBuilder.Head can return first item`` () =
    let test = observe {
        yield 1
        yield 2
        yield 3
...
[<Test>]
let ``Xunit: RxQueryBuilder.ExactlyOne can returns only one item`` () =
    let test = observe { yield 1 }
...
[<Test>]
let ``Xunit: RxQueryBuilder.ExactlyOne throws when source contains more than one item`` () =
    let test = observe {
        yield 1
        yield 2 }
...
[<Test>]
let ``Xunit: Zipping two observable sequences of different types creates a single zipped observable``() =
    let obs1 = Observable.Return 1
    let obs2 = Observable.Return "A"
    let zipped = Observable.zip obs1 obs2
    let result = zipped |> Observable.Wait
...
[<Test>]
let ``Xunit: ZipWith on two observable sequences of different types creates a single zipped observable``() =
    let obs1 = Observable.Return 1
    let obs2 = Observable.Return "A"
    let zipped = Observable.zipWith tuple obs1 obs2
    let result = zipped |> Observable.Wait
...
[<Test>]
let ``Xunit: distinctKey uses the key function to decide whether an element has been seen before``() =
    let result   = ResizeArray()
    let obs      = Observable.ofSeq [ (1,3); (2,5); (2,7); (1,6); (3,5) ]
    let expected = [ (1,3); (2,5); (3,5) ]
    obs |> Observable.distinctKey fst
...
[<Test>]
let ``Xunit: distinctKeyCompare uses the key function and the comparer to decide whether an element has been seen before``() =
    let comparer = { new System.Collections.Generic.IEqualityComparer<int> with
                        member x.Equals(a,b)    = a % 2 = b % 2
                        member x.GetHashCode(a) = a % 2 } // Equivalence based on odd or even
    let result   = ResizeArray()
...
[<Test>]
let ``Xunit: distinctUntilChangedKey uses the key function to decide whether an element has been seen before``() =
    let result   = ResizeArray()
    let obs      = Observable.ofSeq [ (1,3); (2,5); (2,7); (1,6); (3,5) ]
    let expected = [ (1,3); (2,5); (1,6); (3,5) ]
    obs |> Observable.distinctUntilChangedKey fst
...
[<Test>]
let ``Xunit: distinctUntilChangedKeyCompare uses the key function and the comparer to decide whether an element has been seen before``() =
    let comparer = { new System.Collections.Generic.IEqualityComparer<int> with
                        member x.Equals(a,b)    = a % 2 = b % 2
                        member x.GetHashCode(a) = a % 2 }  // Equivalence based on odd or even
    let result   = ResizeArray()
...
[<Test>]
let ``Xunit: ofSeqOn enumerates its sequence on the specified scheduler``() =
    let result    = ResizeArray()
    let items     = [ 1; 2; 3]
    let scheduler = TestScheduler()
...
[<Test>]
let ``Xunit: intervalOn produces a value at the specified rate on the supplied scheduler``() =
    let result    = ResizeArray()
    let scheduler = TestScheduler()
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    ...
    Assert.IsEmpty(result)
    scheduler.AdvanceBy oneSecond; Assert.IsEmpty(result)
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result.Count, 1)
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result.Count, 1)
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result.Count, 2)

[<Test>]
let ``Xunit: throttleOn produces a value at the specified rate on the supplied scheduler``() =
    let result    = ResizeArray()
    use obs       = new Subject<int>()
    let scheduler = TestScheduler()
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    ...
    obs.OnNext(1);                 Assert.IsEmpty(result)
    scheduler.AdvanceBy oneSecond; Assert.IsEmpty(result)
    scheduler.AdvanceBy oneSecond
    Assert.AreEqual(result, [1] )

    obs.OnNext(2)
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result, [1] )
    obs.OnNext(3)
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result, [1] )
    scheduler.AdvanceBy oneSecond; Assert.AreEqual(result, [1; 3] )

[<Test>]
let ``Xunit: combineLatest calls map function with pairs of latest values``() =
    let result   = ResizeArray()
    use obs1     = new Subject<int>()
    use obs2     = new Subject<int>()
    let map (x, y)  = x + (y / 2)
    ...
    obs1.OnNext 3                 
    Assert.AreEqual(result, [ 7; 12; 13 ] )

[<Test>]
let ``Xunit: combineLatestArray produces arrays of latest values``() =
    let result   = ResizeArray()
    use obs1     = new Subject<int>()
    use obs2     = new Subject<int>()
    Observable.combineLatestArray [| obs1; obs2 |]
    ...
    Assert.IsEmpty(result)
    obs2.OnNext 10
    //Assert.AreEqual(List.ofSeq result, [ [| 2; 10 |] ] )
    obs2.OnNext 20                
    //Assert.AreEqual(result, [ [| 2; 10 |]; [| 2; 20 |] ] )
    obs1.OnNext 3                 
    //Assert.AreEqual(result, [ [| 2; 10 |]; [| 2; 20 |]; [| 3; 20 |] ] )

[<Test>]
let ``Xunit: combineLatestSeq produces lists of latest values``() =
    let result   = ResizeArray()
    use obs1     = new Subject<int>()
    use obs2     = new Subject<int>()
    let sources  = [ obs1 :> IObservable<_>; obs2 :> IObservable<_>] |> Seq.ofList
...
[<Test>]
let ``Xunit: combineLatestSeqMap applies map function to latest values``() =
    let result   = ResizeArray()
    use obs1     = new Subject<int>()
    use obs2     = new Subject<int>()
    let sources  = [ obs1 :> IObservable<_>; obs2 :> IObservable<_>] |> Seq.ofList
    ...
    obs2.OnNext 20                
    Assert.AreEqual(result, [ 12; 22 ] )
    obs1.OnNext 3                 
    Assert.AreEqual(result, [ 12; 22; 23 ] )

[<Test>]
let ``Xunit: replay replays all notifications upon subscription``() =
    let result   = ResizeArray()
    use subject  = new Subject<int>()
...
[<Test>]
let ``Xunit: replayBuffer replays only the required count of notifications upon subscription``() =
    let result   = ResizeArray()
    use subject  = new Subject<int>()
...
[<Test>]
let ``Xunit: replayWindowOn replays only the required time range of notifications upon subscription``() =
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    let result    = ResizeArray()
    use subject  = new Subject<int>()
    let scheduler = TestScheduler()
...
[<Test>]
let ``Xunit: replayBufferWindowOn replays only the required time range of notifications upon subscription``() =
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    let result    = ResizeArray()
    use subject  = new Subject<int>()
    let scheduler = TestScheduler()
...
[<Test>]
let ``Xunit: replayBufferWindowOn replays only the required count of notifications upon subscription``() =
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    let result    = ResizeArray()
    use subject  = new Subject<int>()
    let scheduler = TestScheduler()
...
[<Test>]
let ``Xunit: timestampOn uses timestamps from the supplied scheduler``() =
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
    let result    = ResizeArray()
    use subject   = new Subject<int>()
    let scheduler = TestScheduler()
    ...
    Assert.AreEqual(result.[0].Timestamp, firstNotificationAt)
    Assert.AreEqual(result.[1].Timestamp, secondNotificationAt)

[<Test>]
let ``Xunit: Observable.Create should support a simple observable returning fun () -> ()``() =
    let obs =
        Observable.Create(fun (o : IObserver<_>) ->
            o.OnNext("xxx")
            o.OnCompleted()
    ...
    use x = obs.Subscribe(fun result -> Assert.AreEqual(result, "xxx"))
    ()

[<Test>]
let ``Xunit: Observable.subscribeOn should run subscription on another thread`` () =
    let expected  = "Hello World"
    let scheduler = new TestScheduler()
    let result    = ResizeArray()
    let oneSecond = TimeSpan.FromSeconds(1.).Ticks
...
[<Test>]
let ``Xunit: FlatMapAsync should take F# async workflows and flatmap them to observables``() =
    let expected = "<head>fake header</head>"
    let fakeHttpRequest _ =
        async {  return expected }
    let result    = ResizeArray()
    ...
    Assert.AreEqual(result.[0], expected)
    Assert.AreEqual(result.[1], expected)
    Assert.AreEqual(result.[2], expected)

[<Test>]
let ``Xunit: Observable.mapi should be equivalent to Array.mapi`` ()=
    Check.QuickThrowOnFailure <| 
    fun (items : int array) ->
        items
        |> Observable.ofSeq
...
[<Test>]
let ``Xunit: Observable.mapTo always maps to constant`` () =
    Check.QuickThrowOnFailure <|
    fun (xs : int array) (y : int) ->
        Observable.ofSeq xs
        |> Observable.mapTo y
...
[<Test>]
let ``Xunit: filteri should be equivalent to mapi then filter`` () =
    Check.QuickThrowOnFailure <|
    fun (items : int array) ->    
        let predicate i x = (i % 2 = 0) && (x > 0)
...
//[<Test>]
//let ``Xunit: serve subscribes to all created observables`` () =
//    TestSchedule.usage <| fun sch ->
//        Observable.serve 1 (fun () -> Observable.range 0 10)
//        |> Observable.takeUntilOther (Observable.timerSpan (TimeSpan.FromMilliseconds 100.))
//        |> TestSchedule.subscribeTestObserver sch
//        |> TestObserver.nexts
//        |> fun xs -> Assert.AreEqual (xs.Length > 10)
    ...
    //    |> Observable.toEnumerable
    //    |> Seq.toList
    //    |> fun ys -> Assert.AreEqual<int64 list> ([0L..3L], ys)
